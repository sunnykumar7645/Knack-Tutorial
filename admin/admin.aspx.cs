...
public partial class admin_admin : System.Web.UI.Page
{
    ...
    SqlConnection con2 = new SqlConnection(ConfigurationManager.ConnectionStrings["reg1"].ConnectionString);
    SqlCommand com;
    ...
    {
        con.Open();
        String cmd = "select email_address from [admin] where email_address='" + TextBox1.Text.ToString() + "' and password='" + MyEncrypt(TextBox2.Text.ToString()) + "'";
        SqlCommand login = new SqlCommand(cmd, con);
        SqlDataReader read = login.ExecuteReader();
        if (read.Read())
        {
            Session["admin_email_address"] = read.GetValue(0).ToString().Trim();
            Response.Redirect("dashboard.aspx");
        }
        else
        {
